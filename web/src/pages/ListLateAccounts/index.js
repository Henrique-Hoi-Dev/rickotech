...
const ListLateAccounts = ({ accountList, handlerRemoveAccount }) => {
  ...
  useEffect(() => {
    function onLoad() {
      dispatch(findAllLateAccountRequest());
    }
    onLoad();
  }, [dispatch]);
 ...
 //formatção do preço do produto
  ...
  return (
    <Container>
      <Header title="Todas dividas vencidas"/>
         ...
         <Accordion>
            ...
            >
              <Typography sx={{ width: '100%' }}>
                <table key={i} value={contas.id} className="table-list">
                  <thead>
            ...
            <AccordionDetails sx={{display: 'flex', flexDirection: 'column', alignItems: 'center'}}>
              <Typography sx={{ width: '70%' }}>
                {contas.parcela.map((parcelas, i) => (
                  <table key={i} value={parcelas.id} className="table-list">
                    <thead>
                      <tr className="table-title">
                        <td>Valor</td>
                        <td>N parcela</td>
...
export default connect(mapStateToProps, mapDispatchToProps)(ListLateAccounts);
